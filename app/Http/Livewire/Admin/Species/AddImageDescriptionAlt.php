...
class AddImageDescriptionAlt extends Component
{
    use WithPagination;
    use Actions;
    public $description;
    ...
    public function description($id){
        ...
        SpeciesImage::where('id',$id)->update([
    ...
    public function render()
    {
        ...
        $images = SpeciesImage::paginate(10);

        $currentPage = $images->currentPage();
        $pageMin = Request::query('page_min');
        $pageMax = Request::query('page_max');

        
        if ($pageMin && $pageMax) {
            if ($pageMin <= $currentPage && $currentPage <= $pageMax) {
        ...
        foreach ($images as $key => $value) {
            $this->description[$value->id] = $value->description;
            $plantDetails = $this->parseDescription(str_replace(",","",$value->description));
            $this->plantDetails[$value->id] = $plantDetails;
        }

        return view('livewire.admin.species.add-image-description-alt',[
            'images'=>$images
        ]);
