@if ($hasErrors($errors))
    <div {{ $attributes->merge(['class' => 'text-red-600 rounded-lg bg-negative-50 dark:bg-secondary-800 dark:border dark:border-negative-600 p-4']) }}>
...
<!-- @if ($hasErrors($errors))
    <div x-init="() => { var first = document.getElementsByClassName('validation-error').item(0); window.scroll({
        ...
        behavior: 'smooth',
    })  }" {{ $attributes->merge(['class' => 'text-red-600 rounded-lg bg-negative-50 dark:bg-secondary-800 dark:border dark:border-negative-600 p-4']) }}>
        <div class="validation-error flex items-center pb-3 border-b-2 border-negative-200 dark:border-negative-700">
            <x-dynamic-component
                :component="WireUi::component('icon')"
                class="w-5 h-5 text-negative-400 dark:text-negative-600 shrink-0 mr-3"
                name="exclamation-circle"
